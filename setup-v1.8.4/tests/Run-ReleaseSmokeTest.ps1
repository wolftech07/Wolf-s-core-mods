...
try {
    ...
    $releasePlan = @(Get-InstallPlan $context)
    foreach ($entry in $releasePlan) { Assert-Smoke ((Get-Digest $entry.Path) -eq $entry.InstalledHash) ('Payload bytes verified: '+$entry.Target) }
    Invoke-SetupOperation Install $copiedGame $copiedLauncher $payload
    Invoke-SetupOperation Validate $copiedGame $copiedLauncher $payload
    ...
    foreach ($entry in $releasePlan) { Assert-Smoke (!(Test-Path -LiteralPath $entry.Path)) ('Undo restores original absence: '+$entry.Target) }
    Write-Output 'PASS: Release install/reinstall/validate/undo using copies of the selected real launcher dependencies.'
} finally {
