...
namespace TavernNativeMenu
{
    ...
    internal sealed class NativePrompt : IUseVirtualKeyboard
    {
        ...
        private void OpenKeyboard()
        {
            ...
            keyboard.Input = "";
            EnsureCancelKey();
            MelonLogger.Msg("[Tavern In-Game Hub] Native VR input opened. Touch Enter to continue or Cancel to go back.");
        }
        ...
        {
            MelonLogger.Warning("[Tavern In-Game Hub] VR input could not open: " + error.GetType().Name);
            Finish(null);
        ...
        private void Finish(string result)
        {
            ...
            try { RestoreKeyboard(); }
            catch (Exception error) { MelonLogger.Warning("[Tavern In-Game Hub] Keyboard cleanup: " + error.GetType().Name); }
            finally
            {
                value = "";
