...
function Confirm-Compatibility($Context) {
    Write-Step CHECK 'Checking the game, MelonLoader, TavernLib, and Tavern Launcher 1.8.3 / 1.8.4.'
    $launcherHash = Get-Digest $Context.LauncherExe
    if (@('19DA46DFF42DF3328993A1F1A8BE45EA53210A886699FD709A4F8417A5EB65A0','28A59F59518B1D0D741EFCCA65BCC355BF528804F20B8FF7AE9712A1CEF46673') -notcontains $launcherHash) {
        throw 'This setup supports the original Tavern Launcher Client 1.8.3 and 1.8.4 executables. Select a supported release; another version or a modified EXE was found.'
    }
    ...
    if (@('C2229C0B54D883E9502E2CACABE2FF539C8A0826EF47107D5F5381D200D4CC9E','770B2EE118709B6472CA99869FED668D1D850DB55916FB634FB9EEF0A5DA0498','3C02046C9647821EA549F35A113D4F8F48A0130252B7EFC8D83DAE7A98BD6074','B1260921CCAE6F48CE688850402DE19BCF6952CA75C1C073476AE03533586790') -notcontains $libHash) {
        throw 'This TavernLib build has not been checked. Apply the Tavern Launcher 1.8.4 client patch and retry.'
    }
    if ($libHash -ne 'C2229C0B54D883E9502E2CACABE2FF539C8A0826EF47107D5F5381D200D4CC9E') {
        Write-Step WARN 'A previously supported TavernLib build is installed. Use Tavern Launcher 1.8.4 to update the client patch when ready.'
    }
...
function Resolve-Target($Context, [string]$Scope, [string]$Relative) {
    ...
    if ($Context.PSObject.Properties['ServerOnly'] -and $Context.ServerOnly) {
        if ($Scope -ne 'game' -or $normal -ne 'Mods\TavernNativeSocialServer.dll') { throw 'The server-card setup contains an unapproved target.' }
...
function Get-ManagedFileChanges($Context, $Record, [array]$Plan) {
    ...
    foreach ($item in $Plan) {
        ...
        if (!$installed.ContainsKey($item.Path) -and ($item.Scope -ne 'game' -or
            !$item.Target.Replace('/','\').StartsWith('TavernNativeMenu\',[StringComparison]::OrdinalIgnoreCase))) {
            throw 'An update may add files only inside the game TavernNativeMenu folder. Existing files and backups were left untouched.'
        }
    }
    foreach ($path in $installed.Keys) {
        if (!$planned.ContainsKey($path)) { throw 'This release removes files managed by the installed release. Undo that release before installing this one. Existing files and backups were left untouched.' }
    }
    foreach ($item in $Plan) {
        if (!$installed.ContainsKey($item.Path)) {
            # Include new ownership even when an existing file already has the
            # packaged bytes. Undo must still restore its original existence.
            [pscustomobject]@{Scope=$item.Scope;Target=$item.Target;Source=$item.Source;Hash=$item.InstalledHash;ExpectedBefore=(Get-Digest $item.Path);Added=$true}
        } elseif ($installed[$item.Path].InstalledHash -ne $item.InstalledHash) {
...
function Invoke-SetupCore([string]$Action, [string]$Game, [string]$Launcher, [string]$Payload) {
    ...
    if ($record) {
        ...
        # Newly managed support files receive their own original baseline. The
        # transaction journals both kinds, including absence, before any writes.
        foreach ($change in $changes) {
            $path = Resolve-Target $context $change.Scope $change.Target
            if ($change.Added) {
                $backup = $null
                if ($change.ExpectedBefore) {
                    $backup = 'originals\' + [Guid]::NewGuid().ToString('N') + '\added.original'
                    $backupFile = Join-Contained $context.StateRoot $backup
                    [IO.Directory]::CreateDirectory([IO.Path]::GetDirectoryName($backupFile)) | Out-Null
                    [IO.File]::Copy($path,$backupFile)
                    if ((Get-Digest $backupFile) -ne $change.ExpectedBefore) { throw 'An added-file original backup failed verification. No install files were changed.' }
                }
                $record.Files = @($record.Files) + @([pscustomobject]@{Scope=$change.Scope;Target=$change.Target;Existed=[bool]$change.ExpectedBefore;OriginalHash=$change.ExpectedBefore;Backup=$backup;InstalledHash=$change.Hash})
            } else {
                foreach ($entry in @($record.Files)) {
