"""Play Game integration for Tavern Launcher Client 1.8.3.
...
def _validate_game(exe):
    ...
    for relative in required:
        if not os.path.isfile(os.path.join(folder, relative)):
            raise ValueError("Missing " + relative + ". Install the Tavern client patch and run Tavern Native Menu Setup for this game folder.")
    if _digest(os.path.join(folder, required[-1])) not in SUPPORTED_ROOTS:
        raise ValueError("This game build has not been checked for Tavern Native Menu. Use the compatible game patch or an updated mod.")
    if _digest(os.path.join(folder, "Plugins", "TavernLib.dll")) not in SUPPORTED_TAVERN_LIBS:
        raise ValueError("This TavernLib build has not been checked for Tavern Native Menu. Use the 1.8.3 client patch or an updated mod.")
...
def _play_game(self):
    ...
    try:
        ...
        messagebox.showerror("Tavern Native Menu", detail, parent=self)
    finally:
...
def _install():
    ...
    def initialize(self, *args, **kwargs):
        ...
        self._print("Tavern Native Menu enabled. Press Play Game, then select a server in the game.", "ok")
