        ...
        internal static void Apply(ServerSelectionMenu menu)
        {
            ...
            try
            {
                ...
                if (signs + links + removed > 0)
                    MelonLogger.Msg("[Tavern Native Menu] In-game branding: " + signs + " badges, " + links + " Discord signs, " + removed + " sponsor/Vivox renderers updated.");
            }
            catch (Exception error) { MelonLogger.Warning("[Tavern Native Menu] Menu branding: " + error.Message); }
        }
