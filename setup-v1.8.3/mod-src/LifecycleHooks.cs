...
namespace TavernNativeMenu
{
    ...
    internal static class LifecycleHooks
    {
        ...
        public static void Install(HarmonyLib.Harmony harmony)
        {
            ...
            installed = true;
            MelonLogger.Msg("[Tavern Native Menu] Native menu startup and return hooks installed.");
        }
        ...
        private static void RemoveTavernPrefix(HarmonyLib.Harmony harmony, MethodBase target, string methodName)
        {
            ...
            foreach (Patch patch in patches.Prefixes)
            {
                    ...
                    owner.Assembly.GetName().Name == "TavernLib")
                {
                    ...
                    MelonLogger.Msg("[Tavern Native Menu] Restored menu behavior from TavernLib." + methodName + ".");
                }
            }
        }
        ...
        private static async Task LoadNativeMenu()
        {
            try
            {
                ...
                MelonLogger.Msg("[Tavern Native Menu] Opening the original server selection scene.");
                await AltaSceneManager.LoadMainMenuSceneAsync();
            }
            catch (Exception error)
            {
                MelonLogger.Error("[Tavern Native Menu] Could not open the native menu: " + error.GetType().Name + ": " + error.Message);
            }
        }
        ...
        private static async Task ReturnAfterStop(Task stopping)
        {
            try
            {
                ...
                try
                {
                    ...
                    MelonLogger.Msg("[Tavern Native Menu] Returned to the native server selection scene.");
                }
                finally
                {
            ...
            {
                MelonLogger.Error("[Tavern Native Menu] Could not return to the native menu: " + error.GetType().Name + ": " + error.Message);
            }
        }
